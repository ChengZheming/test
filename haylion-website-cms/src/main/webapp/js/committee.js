...
//	专家委员会
	$(function(){		
		  $.ajax({
	           type:"post",
	           url:pathName+"/news/getList.do",	
	           data:{"typeid":13,"langid":1},  
	           success:function(data){        	 
	               var str = "";	               
	              for(var i =0;i<data.newlist.length;i++){
	            	   var imgTime=data.newlist[i].year+'年'+data.newlist[i].month+'月'+data.newlist[i].day+'日';
		 ...
		 var pageSize=1;
			...
			var newsHtml="";
			  $.ajax({
		          ...
		          url:pathName+"/news/getList.do",
		          async:false,  
		          data:{"start":0,"limit":pageSize,"typeid":14,"langid":1},      
		          success:function(data){      
		              total=data.total;
			    ...
			    backFn:function(p){  
			        var start=(p-1)*pageSize;     
				        $.ajax({
				            ...
				            url:pathName+"/news/getList.do",
				            async:false,  
				            data:{"start":start,"limit":pageSize,"typeid":14,"langid":1},
				            success:function(data){	
				            	html="";
				              for(var i =0;i<data.newlist.length;i++){   
				            	  var imgTime=data.newlist[i].year+'年'+data.newlist[i].month+'月'+data.newlist[i].day+'日';
