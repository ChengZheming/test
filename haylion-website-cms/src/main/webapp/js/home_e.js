...
$(function(){
	  $.ajax({
         ...
         data:{"start":0,"limit":4,"typeid":1,"langid":2},  
         success:function(data){
            ...
            for(var i =0;i<data.newlist.length;i++){		
         	   var Monday=data.newlist[i].year+'-'+data.newlist[i].month
                 ...
                 //新闻列表链接点击事件
                 str+='<div class="news-detail"><a class="news-a" href="newsDetails_e.jsp?newsId='+data.newlist[i].id+'&langid=2">'+data.newlist[i].title +'</a></div>';
                 str+='</li>';
...
$(function(){
		  $.ajax({
	         ...
	         data:{"id":16,"langid":2},   //12改为14  xhb 2018.7.18
	         success:function(data){        	 	       
...
$(function(){
	  $.ajax({
         type:"post",
         url:pathName+"/news/getList.do",	
         data:{"start":0,"limit":1,"typeid":16,"langid":2},  
         success:function(data){
             var vd = "";
             for(var i =0;i<data.newlist.length;i++){
            	//html5加入的video标签，poster属性值为URL规定视频下载时显示的图像，或者在用户点击播放按钮前显示的图像。
         	    ...
         	    '" style="width:100%;height:100%;object-fit: fill;">your browser does not support the video tag</video>';
...
$(function(){
	  $.ajax({
         type:"post",
         url:pathName+"/news/getList.do",	
         data:{"start":0,"limit":1,"typeid":6,"langid":2},  
         success:function(data){
             var intro = "";
             for(var i =0;i<data.newlist.length;i++){			            	   		            	        	 		            	   		            	  
