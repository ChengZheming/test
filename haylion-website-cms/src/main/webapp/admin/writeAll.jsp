...
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/static/ueditor/third-party/video-js/video-js.min.css"/>  
<script type="text/javascript" src="${pageContext.request.contextPath}/static/ueditor/third-party/video-js/video.js"></script>  
<script type="text/javascript" charset="gbk" src="${pageContext.request.contextPath}/static/ueditor/ueditor.config.js"></script>
<script type="text/javascript" charset="gbk" src="${pageContext.request.contextPath}/static/ueditor/ueditor.all.min.js"> </script>
<script type="text/javascript" src="${pageContext.request.contextPath}/static/ueditor/ueditor.parse.js"></script>  
...
function submitData(){
    ...
    var langid=$("#langid").combobox("getValue");
       if(langid==null || title==''){
		...
		alert("请选择新闻类别！");
	}else if(workType==null || newTypeId==''){
		alert("请选择工作类别！");
	}else if(workPlace==null || newTypeId==''){
		alert("请选择工作地点！");
	}else if(content==null || content==''){
		alert("请输入内容！");
	}else{
		$.post("${pageContext.request.contextPath}/news/save.do",{'langid':langid,'title':title,'worktype':workType,'workplace':workPlace,'releasedate':tdate,'imagePath':imagePath,'typeid':newTypeId,'content':content,'contentNoTag':UE.getEditor('editor').getContentTxt(),'summary':UE.getEditor('editor').getContentTxt().substr(0,155)},function(result){
			if(result.success){				
				alert("新闻发布成功！");
				resetValue();
			}else{
	...
	// 重置数据
	function resetValue(){
		$("#langid").combobox("setValue","");
		$("#title").val("");
		$("#workType").val("");
		$("#workPlace").val("");
		$("#tdate").val("");
		$("#show").attr('src',"../static/images/tp.png"); 
		$("#newTypeId").combobox("setValue","");
...
<div id="p" class="easyui-panel" title="编写新闻" style="padding: 10px">
 	<table cellspacing="20px">
 		<tr>
   		...
   		<tr>
   			<td width="80px">模块标题：</td>
   			<td><input type="text" id="title" name="title" style="width: 310px;"/></td>
   		</tr>
   		<tr>
