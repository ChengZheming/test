...
function submitData(){
    ...
    var langid=$("#langid").combobox("getValue");
       if(langid==null || title==''){
		...
		$.post("${pageContext.request.contextPath}/news/save.do",{'langid':langid,'title':title,'releasedate':tdate,'imagePath':imagePath,'typeid':newTypeId,'content':content,'contentNoTag':UE.getEditor('editor').getContentTxt(),'summary':UE.getEditor('editor').getContentTxt().substr(0,155)},function(result){
			if(result.success){				
				alert("新闻发布成功！");
				resetValue();
			}else{
